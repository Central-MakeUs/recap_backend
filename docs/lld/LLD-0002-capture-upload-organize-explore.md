...
| --- | --- |
| 상태 | Accepted (개정) |
| 날짜 | 2026-07-13 최초 확정, 2026-07-16 개정(정리 취소 시 InfoCard 처리 방식) |
...
- 2026-07-16 (1차): "정리 취소 시 이미 완료된 항목 처리" 정책을
  뒤집음. 이슈 #13 STAGE3에서는 "취소해도 이미 커밋된 InfoCard는
  유지"(2안)로 임시 채택했으나(아래 "정리(Organize) 처리" 절 참고),
  PM 확인 결과 "취소 시 이미 생성된 InfoCard도 전부 삭제"(1안)로
  확정됐다. 이유: (1) PM 확인 — 취소는 "없었던 일로 되돌리기"를
  의미해야 하며, 부분적으로 결과물이 남는 것은 사용자 기대와 어긋남.
  (2) "취소"라는 단어의 일반적 의미(작업을 중단하고 원상 복구)와의
  일관성 — "일부만 유지"는 취소가 아니라 부분 완료에 가까움. 구현은
  `OrganizeService.cancel()`이 배치를 `CANCELLED`로 바꾼 뒤 해당
  배치의 `InfoCard`를 전부 조회해 원본 S3 오브젝트를 삭제하고 DB에서도
  삭제하는 방식으로 변경(이슈 #13 STAGE4).
...
🔴 **위 문단은 더 이상 유효하지 않다** — "개정 이력" 1차 개정으로
뒤집힘. 취소 시 이미 생성된 InfoCard도 원본 S3 오브젝트와 함께 전부
삭제한다. 락으로 배치를 조회한 직후 상태가 `CANCELLED`인 경우
`completeImage()`는 InfoCard를 만들지 않고 그대로 리턴해, 취소와
동시에 진행 중이던 이미지 처리가 새 InfoCard를 만드는 레이스를 막는다
(같은 비관적 락을 타므로 별도 동시성 장치 불필요).
...
- [x] 정리 취소 시 이미 완료된 항목 처리 — 2026-07-16 개정 이력
      1차로 확정(삭제, 유지 아님). 위 "정리(Organize) 처리" 절 참고.
- [ ] 정리 실패 상세 케이스(AI 분석 실패 외 추가 케이스) 리스트업
- [ ] S3 고아 파일 정리 배치 (ADR-0012 후속)
- [ ] `keywords` 필드 완전 제거 여부
- [ ] `@Async` 처리 방식의 장애 허용 수준 — 트래픽 증가 시 재검토
- [ ] `ORGANIZE_IN_PROGRESS` 체크는 check-then-act 방식이라 이론상
      레이스 가능(더블탭 등). 데이터 손상은 없고 Gemini API 중복 호출
      정도의 낮은 심각도라 지금은 애플리케이션 레벨 체크만 하고 DB
      유니크 제약은 도입하지 않기로 결정. 필요해지면 재검토.
